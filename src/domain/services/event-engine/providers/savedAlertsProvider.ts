...
import { normalizeSavedAlert } from "../normalizers/savedAlertNormalizer";
...
export const savedAlertsProvider: EventProvider = {
  id: "supabase_alerts",
  label: "Supabase Saved Alerts",
  ttlMs: TTL_MS,
  ...
  async load(ctx?: ProviderLoadContext): Promise<GlobalEvent[]> {
    ...
    try {
      const alerts = await supabaseService.listSavedAlerts();
      const events = alerts.map(normalizeSavedAlert);

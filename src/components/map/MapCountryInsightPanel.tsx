  ...
  return (
    <div className="glass-card space-y-3 p-3">
      ...
      <div className="grid grid-cols-2 gap-2 text-[11px]">
        ...
        <div className="rounded-md border border-border/40 bg-secondary/15 p-2">
          <div className="text-muted-foreground">Risk score (recent events)</div>
          <div className="font-medium">{recentEvents.length ? `${riskScore} / 100` : "No recent events"}</div>
        </div>
        <div className="rounded-md border border-border/40 bg-secondary/15 p-2">
          <div className="text-muted-foreground">Saved status</div>
          <div className="font-medium">{savedCount > 0 ? `${savedCount} saved item(s)` : "None saved"}</div>
        </div>
      </div>
      ...
      <div>
        ...
        >
          ...
          {weather ? "Refresh weather" : "Load weather"}
        </button>
      ...
      {recentEvents.length > 0 && (
        <div>
          <div className="mb-1 text-[10px] uppercase tracking-wider text-muted-foreground">Recent events</div>
          <div className="space-y-1">
            {recentEvents.map((e) => (
              ...
              >
                ...
                <span className="text-[10px] uppercase tracking-wider text-muted-foreground">{e.category} · {e.severity} · risk {e.riskScore}</span>
              </button>
            ))}
          </div>
        </div>

...
export function WorldActivityTimeline({ intel, quakes, saved }: Props) {
  const rows = useMemo<Row[]>(() => {
    ...
    for (const q of quakes.slice(0, 30)) list.push({
      id: `q-${q.id}`, title: `M${q.magnitude.toFixed(1)} — ${q.place}`, sub: `USGS · depth ${q.depth.toFixed(1)} km`,
    ...
    for (const a of saved.slice(0, 20)) list.push({
      id: `a-${a.id}`, title: a.title, sub: `${a.type} · ${a.source ?? "saved"}${a.location ? ` · ${a.location}` : ""}`,
  ...
  return (
    <div className="glass-card p-4">
      ...
      />
      {rows.length === 0 ? (
        <div className="rounded-md border border-dashed border-border/50 p-6 text-center text-xs text-muted-foreground">No activity yet.</div>
      ) : (
          ...
          {rows.map((r) => {
            ...
            return (
              <div key={r.id} className="flex items-start gap-2 rounded-md border border-border/40 bg-secondary/20 px-2.5 py-1.5">
                ...
                <span className="shrink-0 text-[10px] tabular-nums text-muted-foreground">{new Date(r.time).toLocaleTimeString()}</span>
              </div>
            );
          })}
        </div>

/**
 * Bundled minimal country dataset — last-resort fallback used only when the
 * backend proxy AND the remote REST Countries API are both unreachable.
 *
 * Contains the 50 countries most likely to appear in intelligence, earthquake
 * and risk contexts so search, map capitals, and compare still work offline.
 *
 * Schema matches the `Country` type from `@/types`.
 */
...
/** Search the demo dataset by name (case-insensitive partial match). */
export function searchDemoCountries(name: string): Country[] {
  const q = name.toLowerCase();
  return DEMO_COUNTRIES.filter(
    (c) =>
